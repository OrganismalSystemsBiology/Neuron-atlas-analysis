...
(
 ...
 int *d_num_regions, int w, int h, int d
  ...
  // Reduce
  temp_storage_bytes = 0;
  cub::DeviceReduce::ReduceByKey(NULL, temp_storage_bytes, d_labels_tmp, d_labels_reg, d_img_tmp, d_maxint_reg, d_num_regions, min_op, image_size);
  assert(temp_storage_bytes < cub_tmp_bytes);
  cub::DeviceReduce::ReduceByKey(d_cub_tmp, temp_storage_bytes, d_labels_tmp, d_labels_reg, d_img_tmp, d_maxint_reg, d_num_regions, min_op, image_size);
...
(
 ...
 int *d_num_regions, int w, int h, int d
  ...
  for(int i = 0 ; i < 6; i++) {
    ...
    std::cout << "SortPairs@hessian " <<temp_storage_bytes <<"___" << cub_tmp_bytes << std::endl;
    //assert(temp_storage_bytes < cub_tmp_bytes); 230512 消した
...
// make ring and nuclear KTR labels //new
...
void Multiply_ 
(
 int *d_dilated_labels, char *d_hessian_pd_nuclear, int *d_nuclear_labels, 
    int imageW,
    int imageH,
    int imageD
 ) {
    ...
    //dim3 blocks(imageW*imageH*imageD,1,1);
    //dim3 threads(1,1,1);

    MultiplyKernel <<<blocks, threads>>>(
        d_dilated_labels,
        d_hessian_pd_nuclear,
        d_nuclear_labels,
        imageW,
        imageH,
        imageD
    ...
    getLastCudaError("Multiply_Kernel() execution failed\n");
  ...
  void Subtract_ 
(
 int *d_dilated_labels, int *d_nuclear_labels, int *d_ring_labels,
    int imageW,
    int imageH,
    int imageD
 ) {
    ...
    getLastCudaError("Subtract_Kernel() execution failed\n");
    checkCudaErrors(cudaPeekAtLastError());
    checkCudaErrors(cudaDeviceSynchronize());
...
void make_ring_nuclear_label_KTR 
(
 int *d_dilated_labels, char *d_hessian_pd_nuclear, int *d_nuclear_labels, int *d_ring_labels,
    int imageW,
    int imageH,
    int imageD
 ) {
