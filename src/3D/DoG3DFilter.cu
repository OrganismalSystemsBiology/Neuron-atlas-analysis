...
__constant__ float c_Kernel[MAX_KERNEL_LENGTH * 5];
__constant__ float c_Kernel_US[MAX_KERNEL_LENGTH];
int radius_kernel[5];
int radius_kernel_US[1];
float sigma_kernel[4];



// Row convolution
// width = 2060 = 103 * 5 * 2^2  = divisible by 4*5
...
// Column convolution
// height = 2048 = 2^11  = divisible by 8*16
// radius should be <= 16*2
// sigma should be < 16*2/2
...
extern "C" int initGaussian3DKernel(const float sigma_xy1, const float sigma_z1, const float sigma_xy2, const float sigma_z2){
  ...
  float *h_kernel = (float *)malloc(5*MAX_KERNEL_LENGTH*sizeof(float));
  ...
  radius_kernel[4] = calcGaussianWeight(&h_kernel[MAX_KERNEL_LENGTH*4], 4.0);

  //cudaMemcpyToSymbol(c_Kernel, h_kernel, 4*MAX_KERNEL_LENGTH * sizeof(float));
  cudaMemcpyToSymbol(c_Kernel, h_kernel, 5*MAX_KERNEL_LENGTH * sizeof(float));

  getLastCudaError("Error: MemcpyToSymbol failed!");

  free(h_kernel);

  return radius_kernel[4];
}
...
extern "C" int calcGaussianWeight_US (float *h_kernel, float sigma) {
    int lw = (int)(TRUNCATE * sigma + 0.5);
    int length = lw * 2 + 1;
    float p, sum;
    sum = 1.;
...
extern "C" int initGaussian3DKernel_US (const float sigma_xy1, const float sigma_z1, const float sigma_xy2, const float sigma_z2){

  float *h_kernel_US = (float *)malloc(1*MAX_KERNEL_LENGTH*sizeof(float));

  radius_kernel_US[0] = calcGaussianWeight_US (&h_kernel_US[0], 1.0); //sigma 0.5 

  //printf("initGaussian3DKernel_US called with sigma: %f\n", 0.5);

  cudaMemcpyToSymbol(c_Kernel_US, h_kernel_US, 1*MAX_KERNEL_LENGTH * sizeof(float));

  getLastCudaError("Error: MemcpyToSymbol failed!");

  free(h_kernel_US);

  return radius_kernel_US[0];
}

__global__ void convolutionLayers3DKernel_US (
    unsigned short *d_Dst,
...
)
{
    ...
    for (int i = LAYERS_HALO_STEPS; i < LAYERS_HALO_STEPS + LAYERS_RESULT_STEPS; i++) {
      s_Data[threadIdx.x][threadIdx.y][threadIdx.z + i * LAYERS_BLOCKDIM_Z] = static_cast<float>(d_Src[i * LAYERS_BLOCKDIM_Z * pitch]);
    }
    ...
    for (int i = 0; i < LAYERS_HALO_STEPS; i++) {
      s_Data[threadIdx.x][threadIdx.y][threadIdx.z + i * LAYERS_BLOCKDIM_Z] = (baseZ + i * LAYERS_BLOCKDIM_Z >= 0) ? static_cast<float>(d_Src[i * LAYERS_BLOCKDIM_Z * pitch]) : 0;
    }
    ...
    for (int i = LAYERS_HALO_STEPS + LAYERS_RESULT_STEPS; i < LAYERS_HALO_STEPS + LAYERS_RESULT_STEPS + LAYERS_HALO_STEPS; i++) {
      s_Data[threadIdx.x][threadIdx.y][threadIdx.z + i * LAYERS_BLOCKDIM_Z]= (baseZ + i * LAYERS_BLOCKDIM_Z < imageD) ? static_cast<float>(d_Src[i * LAYERS_BLOCKDIM_Z * pitch]) : 0;
    }
    ...
    for (int i = LAYERS_HALO_STEPS; i < LAYERS_HALO_STEPS + LAYERS_RESULT_STEPS; i++) {
        ...
        sum = min(max(sum, 0.0f), 65535.0f);
        d_Dst[i * LAYERS_BLOCKDIM_Z * pitch] = static_cast<unsigned short>(sum);
    }
}


extern "C" void convolutionLayers3D_US (
    unsigned short *d_Dst,
    unsigned short *d_Src,
    int imageW,
    int imageH,
...
)
{
    ...
    convolutionLayers3DKernel_US<<<blocks, threads>>>(
        d_Dst,
        d_Src,
        imageW,
        imageH,
    ...
    getLastCudaError("convolutionLayers3DKernel_US() execution failed\n");
}



extern "C" void GaussianZFilter(unsigned short *d_img, unsigned short *d_result, const int sigma_num, const int width, const int height, const int depth) {
  


  convolutionLayers3D_US (d_result, d_img, width, height, depth, 4, radius_kernel[4]); // z sigma = 1 -> lw = 2

  
  getLastCudaError("Error: convolutionLayers3D() kernel execution FAILED!");
  //checkCudaErrors(cudaDeviceSynchronize());
}
